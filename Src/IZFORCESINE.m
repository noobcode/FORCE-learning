...
%} 
    ...
    high_dimensional_temporal_signal = 0; 
    ...
    target_frequency = 5;       
    target_phase = 0;   
    target_length = 15000; % in milliseconds
    ...
    N = 2000; %2000 % Number of neurons
    a = 0.01; % adaptation reciprocal time constant
    b = -2; % resonance parameter
    vreset = -65; % reset voltage 
    d = 200;   %adaptation jump current (200 IZFORCESINE) (0 or negative for low-frequency signals)
    C = 250;  %capacitance 
    ...
    %original_zx = sin(2*pi*target_frequency*tt/1000 + target_phase);
    %original_zx = sqrt((tt)) - 0.5 * (tt);
    %original_zx = sqrt(tt) - 0.5 * (tt).^2 +  0.2 * (tt).^3;
    original_zx = (sin(2*pi*target_frequency*(1:1:nt)*dt/1000 + target_phase)); % clopath sine

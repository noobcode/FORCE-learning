function [imin, icrit, icrit_2, nt] = set_simulation_time(training_setting, dt, iteration_per_target_cycle, target_lengths)
    ...
    elseif training_setting == 9
        %% hdts train decelerate (0.5) than accelerate (2)
        T = round(target_lengths(1)/dt);
        imin = T;
        icrit = imin + 2*T;
        icrit_2 = icrit + T;
        nt = icrit_2 + 2*T + 2*(round(T/2));
    elseif training_setting == 10
        %% hdts inverse replay
        T = round(target_lengths(1)/dt);
        imin = T;
        icrit = imin + 2*T;
